...
\begin{standaloneframe}
  \begin{tikzpicture}
    ...
    \onslide<+->{\node[yshift=-1mm,anchor=north west] (theta) at (alphasample.west|-preimages.south) {$\theta := \frac{\pi}{4}(-1)^{x_n}\sum_{i=1}^{n-1}(x_i-x_i')(4b_i+\alpha_i)$};}
    ...
    \onslide<+->{\node[anchor=north, yshift=-2mm, draw=blue, fill=blue!20, rounded corners, inner sep=4pt, rectangle] at ($(theta.south west)!.5!(belowcircuit)$) {{%
          \onslide<+->{\textcolor{blue}{$\ket{0}\ket{0}$}}%
          \onslide<+->{\textcolor{red!80!black}{$\Rightarrow \sum_x\ket{x}\ket{0}$}}%
